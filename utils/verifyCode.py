...
import pytesseract
from config.setting import PICTURE_DIR


def verifyCode(driver, ElementType, ElementValue):
    """
    :param driver: 实例化webdriver
    :param ElementType: id,class,name,css,xpath...
    :param ElementValue: 元素
    :return: 识别二维码字符串
    """
    IMAGE_PATH = os.path.join(os.path.join(PICTURE_DIR, 'verifycode'), 'picture.png')
    FRAME_PATH = os.path.join(os.path.join(PICTURE_DIR, 'verifycode'), 'frame4.png')
    driver.save_screenshot(IMAGE_PATH)  # 截取当前网页
    location = driver.location(ElementType, ElementValue)  # 获取验证码x,y轴坐标
    size = driver.size(ElementType, ElementValue)  # 获取验证码的长宽
